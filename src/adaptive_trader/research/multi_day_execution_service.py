...
from adaptive_trader.microstructure.campaign import MicrostructureDatasetCampaign, load_campaign
...
class MultiDayExecutionEconomicsService:
    def build(
        ...
        *,
        ...
        validate_new_campaign(
            campaign,
            consumed,
            minimum_session_start=_git_commit_time(commit),
        )
        catalog = load_execution_policy_catalog(policy_catalog_path)
        experiment_id = (
            f"multi-day-execution-economics-{datetime.now(tz=UTC).strftime('%Y%m%dT%H%M%SZ')}-"
    ...
    def _report(
        ...
        text = f"""# Multi-day execution economics — Sprint 4A.3.1
